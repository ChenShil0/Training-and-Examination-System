    ...
    <el-table :data="pagination.records" border>
      ...
      <el-table-column fixed="right" label="操作" width="175">
        <template slot-scope="scope">
          <el-button @click="toPart(scope.row.examCode, scope.row.source)" type="primary"> 查看及格率</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
      :current-page="pagination.current" :page-sizes="[4, 8, 10]" :page-size="pagination.size"
...
export default {
  ...
  methods: {
    ...
    toPart(examCode, source) { //跳转到分段charts页面
      this.$router.push({ path: '/averagePart', query: { examCode: examCode, source: source } })
    }
  },
};
</script>
