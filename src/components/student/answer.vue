  ...
  <div id="answer">
    ...
    <div class="flexarea">
      ...
      <transition name="slider-fade">
        <div class="left" v-if="slider_flag">
          ...
          <div class="l-bottom">
            ...
            <el-button type="primary" style="width: 100%;font-size: 22px;margin: 25px 0 0 0;" @click="finishExam()">
              结束考试</el-button>
          </div>
        </div>
      </transition>
      ...
      <transition name="slider-fade">
        <div class="right">
          <div class="title">
            <p style="font-size: 22px;"><i class="iconfont" style="font-size: 28px;"></i> {{
              title }}</p>
          ...
          <div class="operation">
            <ul class="end">
              ...
              <li @click="next()" style="font-size: 24px;">
                <span>下一题 </span><i class="iconfont icon-r-right" style="font-size: 32px;margin-left: 5px;"></i>
              </li>
            </ul>
          </div>
        </div>
...
var uniqueList_multi = new Set();
var uniqueList_fill = new Set();
var uniqueList_judge = new Set();
export default {
  data() {
    return {
    ...
    commit() {
      ...
      topic1Answer.forEach((element, index) => {
        ...
        if (element != null) {
          ...
          if (right != this.topic[1][index].rightAnswer) {
            console.log("选择")
            uniqueList_multi.add(this.topic[1][index].questionId)
          }
        }
      });
      ...
      fillAnswer.forEach((element, index) => {
        ...
        let allElementsEmpty = element.every(element => element === null || element === undefined || element === "");
        if (allElementsEmpty == false) {
          element.forEach((inner) => {
            ...
            if (this.topic[2][index].answer !== inner) {
              //console.log("填空")
              uniqueList_fill.add(this.topic[2][index].questionId)
              //console.log(this.topic[2][index].questionId)
            }
          });
        }
      ...
      topic3Answer.forEach((element, index) => {
        ...
        if (element != null) {
          ...
          if (right == this.topic[3][index].answer) {
            ...
            console.log("判断")
            uniqueList_judge.add(this.topic[3][index].questionId)
          }
        }
      ...
      this.$axios({
        ...
        if (uniqueList_multi.size === 0) {
          console.log('Set is empty, not sending request.');
        } else {
          // 将 Set 转换为 Array
          let uniqueArray_multi = Array.from(uniqueList_multi);
          let uniqueArray_fill = Array.from(uniqueList_fill);
          let uniqueArray_judge = Array.from(uniqueList_judge);


          for (let i = 0; i < uniqueArray_multi.length; i++) {
            this.$axios({
              url: "/api/studentwrong",
              method: "put",
              data: {
                studentId: this.userInfo.id, // 学号
                wrongmulti: uniqueArray_multi[i], // 错题id
              },
            })
          }

          for (let i = 0; i < uniqueArray_fill.length; i++) {
            this.$axios({
              url: "/api/studentwrong",
              method: "put",
              data: {
                studentId: this.userInfo.id, //学号
                wrongfill: uniqueArray_fill[i],//错题id
              },
            })
          }

          for (let i = 0; i < uniqueArray_judge.length; i++) {
            this.$axios({
              url: "/api/studentwrong",
              method: "put",
              data: {
                studentId: this.userInfo.id, //学号
                wrongjudge: uniqueArray_judge[i],//错题id
              },
            })
          }
        }
...
.operation .end li:nth-child(2) {
  ...
  align-items: center;
  background-color: rgb(67, 112, 195);
  border-radius: 50%;
  width: 50px;
  height: 50px;
  color: #fff;
