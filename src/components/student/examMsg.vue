...
<template>
  <div id="msg">
    <div class="title">
      <span :style="{ fontWeight: 'bold', fontSize: '25px' }">试卷列表</span>
      <span :style="{ fontWeight: 'bold', fontSize: '25px' }">/ {{ examData.source }}</span>
    </div>
    <div class="wrapper">
      <ul class="top">
      ...
      <ul class="bottom">
        ...
        <li class="right"><el-button style="font-size: 22px;" @click="toAnswer(examData.examCode)">{{
          $store.state.isPractice == true ? ' 开始练习' : ' 开始考试' }}</el-button></li>
      </ul>
      <ul class="info">
        <li @click="dialogVisible = true"><a href="javascript:;"><i class="iconfont"></i>考生须知</a></li>
      </ul>
    </div>
    <div class="content">
