...
<template>
  <div class="table">
    <h3 style="color: white;font-size:28px">我的分数</h3>
    <section class="content-el">
      <el-table ref="filterTable" :data="score" v-loading="loading">
        ...
        <el-table-column prop="subject" label="课程名称" width="340" filter-placement="bottom-end">
        ...
        <el-table-column label="是否及格" width="140">
          <template slot-scope="scope">
            <el-tag :type="scope.row.ptScore == 1 ? 'success' : 'danger'" style="font-size: 18px;">
              <i class="iconfont" v-if="scope.row.ptScore == 1" style="font-size: 24px;"></i>
              <i class="iconfont" v-if="scope.row.ptScore != 1" style="font-size: 24px;"></i>
