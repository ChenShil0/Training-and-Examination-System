    ...
    <div class="wrapper">
      ...
      <ul class="paper" v-loading="loading">
        <li class="item" v-for="(item, index) in pagination.records" :key="index" @click="toExamMsg(item)">
          ...
          <div class="info">
            <span class="iconfont icon-kaoshishijian">练习时间：{{ item.examDate.substr(0, 10) }}</span>
            <span class="iconfont icon-xianshi" v-if="item.totalTime != null">限时{{ item.totalTime }}分钟</span>
