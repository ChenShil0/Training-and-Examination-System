...
import com.examsys.oes.serviceimpl.ScoreServiceImpl;
...
//    分页
    ...
    @GetMapping("/Scores/{examname}")
    public ApiResult findByExamCode(@PathVariable("examname") String examname) {
        List<Score> scores = scoreService.findByExamname(examname);
        if (!scores.isEmpty()) {
            return ApiResultHandler.buildApiResult(200,"查询成功",scores);
        } else {
            return ApiResultHandler.buildApiResult(400,"查询失败",scores);
        }
    }
}
